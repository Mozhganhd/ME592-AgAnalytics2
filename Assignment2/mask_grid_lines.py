...
def mask_grid_lines(original_image,coordinates):
    ...
    e1,e2,e3,e4=0,0,0,0
    print(len(vertical_coordinates))
    print(len(horizontal_coordinates))
    ...
    horizontal_coordinates=sorted(horizontal_coordinates)    
    vertical_coordinates=sorted(vertical_coordinates)  
    mask_lines=np.zeros_like(image_copy[:,:,0])
    ...
    for i in horizontal_coordinates:
        if(i>=e3 and i<=e4):
            ...
            mask_lines=cv2.line(mask_lines,(0,i),(img.shape[1],i),(255,255,255),3)
    for i in vertical_coordinates:
        if(i>=e1 and i<=e2):
            image_copy=cv2.line(image_copy,(i,0),(i,img.shape[0]),color,2)
            mask_lines=cv2.line(mask_lines,(i,0),(i,img.shape[0]),(255,255,255),3)
    restored = cv2.inpaint(img, mask_lines, 1, cv2.INPAINT_NS)
    luv=random.randint(1,10000)
    # cv2.imwrite('test'+str(luv)+'.jpg',restored)
    ...
    original_image=np.zeros_like(original_image)
    original_image[coordinates[0][0]:coordinates[0][0]+img_coordinate,coordinates[0][1]:coordinates[1][1]]=restored
